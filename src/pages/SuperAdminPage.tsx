...
const statusVariant: Record<StatusKey, 'success' | 'warning' | 'destructive' | 'muted'> = {
  active: 'success',
  trial: 'warning',
  expired: 'destructive',
  disabled: 'muted',
...
function clinicStatus(clinic: Clinic): StatusKey {
  ...
  const subscriptionActive =
    !!clinic.subscription_ends_at && new Date(clinic.subscription_ends_at).getTime() > Date.now()
  if (subscriptionActive) return 'active'
  if (!clinic.trial_ends_at) return 'active'
  return new Date(clinic.trial_ends_at).getTime() > Date.now() ? 'trial' : 'expired'
}
...
export default function SuperAdminPage() {
  ...
  const [editTarget, setEditTarget] = React.useState<ClinicWithUsers | null>(null)
  ...
  const filtered = React.useMemo(() => {
    ...
    return clinics.filter((clinic) => {
      if (statusFilter !== 'all' && clinicStatus(clinic) !== statusFilter) return false
      if (!q) return true
  ...
  const counts = React.useMemo(() => {
    const base: Record<StatusKey, number> = { active: 0, trial: 0, expired: 0, disabled: 0 }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground">كل العيادات المسجّلة في النظام</p>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        ...
        <StatCard label="تجريبي" value={counts.trial} className="text-warning" />
        <StatCard label="منتهي" value={counts.expired} className="text-destructive" />
        <StatCard label="معطل" value={counts.disabled} className="text-muted-foreground" />
      </div>

      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2">
          {(['all', 'active', 'trial', 'expired', 'disabled'] as const).map((key) => (
            <Button
              key={key}
              size="sm"
      ...
      <div className="flex flex-col gap-3">
        {isLoading ? (
                ...
                <CardContent className="flex flex-col gap-3 pt-5 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex flex-col gap-1 text-xs text-muted-foreground sm:items-end">
                    ...
                    {clinic.subscription_ends_at ? (
                      <p>ينتهي الاشتراك: {formatDate(clinic.subscription_ends_at)}</p>
                      ...
                      <p>
                        {status === 'expired' ? (
                          ...
                          <>
                            تنتهي التجربة: {formatDate(clinic.trial_ends_at)} (باقي{' '}
                            <span className="font-semibold text-warning">{daysRemaining(clinic.trial_ends_at)}</span>{' '}
                            يوم)
                          </>
                        )}
                      </p>
                    ...
                    <p>آخر ظهور: {relativeArabic(admin?.last_seen_at ?? null)}</p>
                  </div>

                  <div className="flex items-center gap-2">
                    <Button size="sm" variant="outline" onClick={() => setEditTarget(clinic)}>
                    ...
                    {admin && (
                      <Button
                        ...
                        onClick={() => setUserActive.mutate({ userId: admin.id, isActive: !admin.is_active })}
                      >
                        {admin.is_active ? <PowerOff className="size-4" /> : <Power className="size-4" />}
          ...
          <p className="p-8 text-center text-muted-foreground">لا توجد عيادات مطابقة</p>
        )}
      </div>

      <EditClinicDialog clinic={editTarget} onOpenChange={(open) => !open && setEditTarget(null)} />
      ...
      <DialogContent>
        ...
        <div className="flex flex-col gap-4">
          <div>
            ...
            <Input id="clinic_name" value={name} onChange={(e) => setName(e.target.value)} />
          </div>

          <div className="flex items-center justify-between rounded-lg border border-border p-3">
            <span className="text-sm font-medium">حالة الحساب</span>
            <Button size="sm" variant={isActive ? 'default' : 'destructive'} onClick={() => setIsActive((v) => !v)}>
              {isActive ? 'مفعل' : 'معطل'}
            </Button>
          </div>

          <div>
            <Label htmlFor="subscription_ends_at">تاريخ انتهاء الاشتراك (اختياري)</Label>
            <Input
            ...
            />
            <p className="mt-1 text-xs text-muted-foreground">اتركه فارغًا لو مفيش اشتراك مدفوع بتاريخ محدد</p>
          </div>

          <div>
            <Label htmlFor="trial_ends_at">تاريخ انتهاء الفترة التجريبية</Label>
            <Input id="trial_ends_at" type="date" value={trialEndsAt} onChange={(e) => setTrialEndsAt(e.target.value)} />
            <p className="mt-1 text-xs text-muted-foreground">
              تُستخدم فقط إذا لم يكن هناك اشتراك مدفوع — اتركها فارغة لحساب غير محدود
            </p>
          </div>
...
function AddClinicDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
    ...
    >
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>سيتم إنشاء حساب مدير للعيادة مباشرةً — احفظ كلمة المرور لمشاركتها معه</DialogDescription>
        </DialogHeader>

        <div className="flex flex-col gap-4">
          <div>
            <Label htmlFor="add_full_name">الاسم الكامل</Label>
            <Input id="add_full_name" value={fullName} onChange={(e) => setFullName(e.target.value)} />
          </div>
          <div>
            <Label htmlFor="add_email">البريد الإلكتروني</Label>
            <Input id="add_email" type="email" dir="ltr" value={email} onChange={(e) => setEmail(e.target.value)} />
          </div>
          <div>
            <Label htmlFor="add_password">كلمة المرور</Label>
            <Input id="add_password" type="text" dir="ltr" value={password} onChange={(e) => setPassword(e.target.value)} />
          </div>

          <div className="rounded-lg border border-border p-3">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">فترة تجريبية مجانية</span>
              <Button size="sm" variant={isTrial ? 'default' : 'outline'} onClick={() => setIsTrial((v) => !v)}>
            ...
            {!isTrial && <p className="mt-2 text-xs text-muted-foreground">حساب دائم — بدون حد أقصى للفترة التجريبية</p>}
          </div>

          <div>
            <Label htmlFor="add_subscription_ends_at">تاريخ انتهاء الاشتراك (اختياري)</Label>
            <Input
